...
@st.cache_resource
def get_google_genai_client(api_key: str):
    """Cached client for the newer google-genai SDK"""
...
def extract_video_frame_thumbnails(video_bytes: bytes, max_frames: int = 3) -> List[str]:
    ...
    try:
        ...
        # Use delete=False to avoid permission errors on some OSs (Windows) when file is open
...
@st.cache_data
def _generate_blip_caption_cached(image_bytes: bytes) -> Optional[str]:
    """Cached worker for BLIP captioning"""
    try:
        from PIL import Image
        from io import BytesIO
...
def generate_blip_caption(image) -> Optional[str]:
    try:
        from io import BytesIO
        buf = BytesIO()
        # Convert PIL image to bytes for caching key
        image.save(buf, format='PNG')
        return _generate_blip_caption_cached(buf.getvalue())
    except Exception as e:
        logger.info(f"BLIP wrapper failed: {e}")
        return None
