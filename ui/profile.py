...
def show_profile_page():
    ...
    with col_header2:
        ...
        st.markdown(
            f"**Account Type:** {'🔐 Google OAuth' if is_oauth else '🔐 Traditional Login'}"
        )
        if st.button(
            "Edit Profile",
            key="edit_profile_btn",
            help="Edit your profile information and preferences.",
        ):
            with st.popover("Edit Profile", use_container_width=True):
                new_name = st.text_input(
                    "Name", value=user_info.get("name", ""), key="edit_name"
                )
                new_avatar = st.text_input(
                    "Avatar URL",
                    value=user_info.get("avatar_url", ""),
                    key="edit_avatar",
                )
    ...
    tab1, tab2, tab3, tab4 = st.tabs(
        ["📊 Statistics", "⚙️ Settings", "🎨 Preferences", "🔒 Security"]
    )
    ...
    with tab1:
        ...
        with col1:
            st.metric(
                "Total Messages",
                total_messages,
                help="Total number of messages exchanged in this session.",
            )
        with col2:
            st.metric(
                "Topics Learned",
                stats.get("total_topics", 0),
                help="Unique topics learned by the AI in this session.",
            )
        with col3:
            st.metric(
                "Models Used",
                stats.get("models_tracked", 0),
                help="Number of different AI models used.",
            )
        with col4:
            st.metric(
                "Conversations",
                stats.get("total_conversations", 0),
                help="Total separate conversations in this session.",
            )
        ...
        if stats.get("model_strengths"):
            for i, model_stat in enumerate(stats["model_strengths"][:5], 1):
                ...
                with col_stats:
                    ...
                    st.caption(
                        f"Success: {model_stat['success_rate']}% ({model_stat['success']}/{model_stat['total']})"
                    )

        st.markdown("### 📈 Activity Timeline")
        joined_date = st.session_state.get(
    ...
    with tab2:
        ...
        if not is_oauth:
            ...
            new_name = st.text_input(
                "Display Name", value=user_data.get("name", st.session_state.username)
            )
            ...
            st.info(
                "🔒 Profile information is managed by Google OAuth and cannot be edited here."
            )
    ...
    with tab3:
        ...
        theme = st.selectbox(
            "Theme", ["Dark"], index=0, help="Dark mode is now enforced."
        )
        ...
        timezone = st.selectbox(
            "Timezone", ["UTC", "EST", "PST", "GMT", "IST"], index=0
        )

        notifications = st.checkbox(
            "Enable Notifications", value=prefs.get("notifications", True)
        )
    ...
    with tab4:
            ...
            with st.form("change_password_form"):
                ...
                if submit:
                    if not all([current_pwd, new_pwd, confirm_pwd]):
                        ...
                        users = load_user_credentials()
                        if users.get(st.session_state.username, {}).get(
                            "password"
                        ) == hash_password(current_pwd):
                            users[st.session_state.username]["password"] = (
        ...
        st.info(
            f"🟢 Current session started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
