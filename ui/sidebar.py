...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        auth_type = (
            "🔐 Google" if "google_oauth_token" in st.session_state else "🔐 Login"
        )
        ...
        display_labels = [
            f"{PROVIDER_ICONS.get(m[2], '⚪')} {m[0]}" for m in filtered_models_list
        ]
        ...
        with st.container():
            ...
            if caps:
                badges_html = "".join(
                    f"<span style='background:#334155; color:#e2e8f0; padding:2px 6px; border-radius:4px; font-size:0.75rem; margin-right:4px;'>{c}</span>"
                    for c in caps
                )
                st.markdown(
        ...
        with st.expander(f"🔑 API Keys ({configured_keys}/6)", expanded=False):
            ...
            st.session_state.google_api_key = st.text_input(
                "Google API Key", value=st.session_state.google_api_key, type=key_type
            )
            st.session_state.openai_api_key = st.text_input(
                "OpenAI API Key", value=st.session_state.openai_api_key, type=key_type
            )
            ...
            st.session_state.xai_api_key = st.text_input(
                "xAI API Key", value=st.session_state.xai_api_key, type=key_type
            )
        ...
        with st.expander("🧠 Brain Mode", expanded=False):
            ...
            if enable_brain:
                ...
                st.session_state.enable_internet = st.checkbox(
                    "Internet Search",
                    value=st.session_state.get("enable_internet", True),
                )
                st.session_state.brain_auto_select = st.checkbox(
                    "Auto-Select Models",
                    value=st.session_state.get("brain_auto_select", True),
                )
                ...
                with c2:
                    anthropic = st.checkbox(
                        "Claude",
                        value=False,
                        disabled=not bool(st.session_state.anthropic_api_key),
                    )
                    together = st.checkbox(
                        "Llama",
                        value=False,
                        disabled=not bool(st.session_state.together_api_key),
                    )
        ...
        with st.expander("📎 Multimodal", expanded=False):
            if "multimodal_voice_integrator" not in st.session_state:
                st.session_state.multimodal_voice_integrator = (
            ...
            st.session_state.voice_mode = st.toggle(
                "Voice Mode", value=st.session_state.get("voice_mode", False)
            )
            st.session_state.auto_speak = st.checkbox(
                "Auto-Speak", value=st.session_state.get("auto_speak", True)
            )
