...
def search_internet(query: str, max_results: int = 5) -> List[Dict[str, str]]:
    ...
    try:
        ...
        return [
            {
                "title": "Error",
                "snippet": "DuckDuckGo search requires: pip install duckduckgo-search",
                "url": "",
            }
        ]
    except Exception as e:
        return [
            {"title": "Search Error", "snippet": f"Search failed: {str(e)}", "url": ""}
        ]
...
class LearningBrain:
    ...
    def __init__(self):
        self.knowledge_base: Dict[str, List[KnowledgeEntry]] = (
            {}
        )  # Learned facts indexed by topic
    ...
    def _is_successful_response(
        self, response: Dict[str, Any], normalized_text: Optional[str] = None
    ) -> bool:
        normalized_text = (
            normalized_text
        ...
        for response in model_responses:
            ...
            if response_time > 0:
                ...
                perf["avg_response_time"] = (
                    old_avg * (perf["total"] - 1) + response_time
                ) / perf["total"]
                perf["min_response_time"] = min(
                    perf.get("min_response_time", float("inf")), response_time
                )
                perf["max_response_time"] = max(
                    perf.get("max_response_time", 0), response_time
                )
            ...
            if success:
                ...
                perf["avg_length"] = (
                    current_avg * (total_success - 1) + response_length
                ) / total_success
        ...
        if successful_answers:
            ...
            for keyword in keywords:
                ...
                entry = KnowledgeEntry(
                    ...
                    or [r.get("provider", "unknown") for r in model_responses],
                )
                if (
                    kb_entries
                    and kb_entries[-1].query == entry.query
                    and kb_entries[-1].answers == entry.answers
                ):
                    continue
    ...
    def recommend_models(self, query: str, available_models: List[str]) -> List[str]:
        ...
        for model in available_models:
            topic_score = sum(
                self.topic_expertise.get(keyword, {}).get(model, 0)
                for keyword in keywords
            )
            perf_score = 0
    ...
    def summarize_model_strengths(self) -> List[Dict]:
        ...
        for model, perf in self.model_performance.items():
            success_rate = (
                (perf["success"] / perf["total"] * 100) if perf["total"] else 0
            )
            top_topics = []
    ...
    def get_related_knowledge(self, query: str, limit: int = 3) -> List[Dict]:
        ...
        related.sort(
            key=lambda x: x.timestamp if hasattr(x, "timestamp") else "", reverse=True
        )
    ...
    def get_learning_stats(self) -> Dict:
        ...
        for provider, perf in self.model_performance.items():
            ...
            stats["model_performance"][provider] = {
                ...
                "min_response_time": (
                    round(perf.get("min_response_time", 0), 2)
                    if perf.get("min_response_time", float("inf")) != float("inf")
                    else 0
                ),
        ...
        # Top topics
        top_topics = sorted(
            self.knowledge_base.items(), key=lambda x: len(x[1]), reverse=True
        )[:10]
        stats["top_topics"] = [
            {"topic": topic, "count": len(entries)} for topic, entries in top_topics
        ]
    ...
    def export_knowledge(self) -> str:
        ...
        export_data = {
            ...
            "conversation_history": [
                asdict(record) for record in self.conversation_history
            ],
    ...
    def import_knowledge(self, json_data: str):
        ...
        try:
            ...
            self.conversation_history = [
                ConversationRecord(**record) for record in history
            ]
            return True
        except Exception:
            return False
