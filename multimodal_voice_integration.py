...
class MultimodalVoiceIntegrator:
    ...
    def _process_uploads(self, uploaded_files) -> Dict:
        ...
        for uploaded_file in uploaded_files:
            try:
                ...
                results["errors"].append({"file": uploaded_file.name, "error": str(e)})
    ...
    def create_voice_settings(self) -> VoicePreference:
        ...
        with st.expander("Configure Voice", expanded=False):
            ...
            with col1:
                ...
                gender = st.radio("Voice Gender", [g.value for g in VoiceGender])

                profile = st.selectbox("Voice Profile", [p.value for p in VoiceProfile])

            with col2:
                speed = st.slider("Speech Speed", 0.5, 2.0, 1.0, 0.1)

                accent = st.selectbox(
                    "Accent", ["standard", "british", "indian", "australian"]
                )
    ...
    def create_text_to_speech_interface(
        self, preferences: VoicePreference
    ) -> Optional[bytes]:
        ...
        with st.expander("Convert Text to Speech", expanded=False):
            ...
            with col1:
                provider = st.selectbox(
                    "TTS Provider", ["google", "elevenlabs", "azure", "openai"]
                )

            with col2:
                format_type = st.selectbox("Output Format", ["mp3", "wav", "ogg"])
                ...
                if st.button("🎵 Generate Speech", key="tts_generate"):
                    try:
                        ...
                        audio_bytes, metadata = tts.synthesize(
                            text_input, output_format=format_type
                        )
    ...
    def create_speech_to_text_interface(
        ...
        with st.expander("Convert Speech to Text", expanded=False):
            ...
            with col1:
                provider = st.selectbox(
                    "STT Provider", ["google", "azure", "openai"], key="stt_provider"
                )
            ...
            if st.button("⏹️ Stop Session", key="voice_stop"):
                if hasattr(st.session_state, "voice_session_id"):
                    ...
                    if session_data:
                        ...
                        st.info(
                            f"Session ended. Duration: {stats.get('duration', 0):.2f}s"
                        )
    ...
    def display_multimodal_statistics(self):
        ...
        if self.processing_stats["file_types_processed"]:
            ...
            for file_type, count in self.processing_stats[
                "file_types_processed"
            ].items():
