...
# Mock modules to avoid import errors for external dependencies
...
# Ensure patched targets exist in the module even if it is empty
import ui.chat_utils
if not hasattr(ui.chat_utils, "get_internet_search_engine"):
    ui.chat_utils.get_internet_search_engine = MagicMock()
if not hasattr(ui.chat_utils, "handle_google_provider"):
    ui.chat_utils.handle_google_provider = MagicMock()
...
def test_generate_standard_response_google(mocker):
    ...
    # Note: using mocker fixture from pytest-mock if available, otherwise use patch
    with patch("ui.chat_utils.handle_google_provider", return_value="Google Response") as mock_handle:
        response = generate_standard_response(
            provider="google",
            model_name="gemini",
            api_keys={"google": "key"},
