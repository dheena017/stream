...
def test_encryption_on_disk(temp_env):
    ...
    # Fernet encrypted strings start with gAAAA
...
def test_retention_cleanup(temp_env):
    ...
    # Note: We manually insert here to bypass timestamp setting in save_message if it enforces current time
    # But we need to make sure the manual insertion mimics the encrypted format if we want get_conversation_messages to work
    # However, cleanup usually just checks timestamp.
    # Let's encrypt it manually using EncryptionManager or just insert a dummy encrypted string if cleanup doesn't decrypt.
    # Cleanup likely just checks timestamp.

    # We need to make sure 'init_db' created the table

    c.execute("INSERT INTO messages (conversation_id, role, content, meta_json, timestamp) VALUES (?, ?, ?, ?, ?)",
              (conv_id1, "user", "gAAAA_fake_encrypted_old", "{}", old_date))
    c.execute("INSERT INTO messages (conversation_id, role, content, meta_json, timestamp) VALUES (?, ?, ?, ?, ?)",
              (conv_id2, "user", "gAAAA_fake_encrypted_old", "{}", old_date))
...
def test_export_data(temp_env):
    ...
    # Verify the message content in the export is DECRYPTED
